pub mod diesel {
    ...
    pub type DieselPool<T> = Arc<r2d2::Pool<r2d2_diesel::ConnectionManager<T>>>;
    ...
    pub type DieselPooledConnection<T> = r2d2::PooledConnection<r2d2_diesel::ConnectionManager<T>>;
    // Commented because of:
    // warning[E0122]: trait bounds are not (yet) enforced in type definitions
    ...
    impl<T: diesel::Connection> DieselMiddleware<T> {
        ...
        /// Creates a new pooled connection to the given sql server. The URL is
        /// in the format:
        ///
        /// ```{none}
        ...
        /// ```
        ///
        /// Returns `Err(err)` if there are any errors connecting to the sql
        /// database.
        pub fn new(connection_str: &str) -> Result<DieselMiddleware<T>, Box<Error>> {
            Self::new_with_config(connection_str, r2d2::Config::default())
        }

        /// Creates a new connection pool, with the ability to set your own r2d2
        /// configuration.
    ...
    pub trait DieselReqExt<T: 'static + diesel::Connection> {
        /// Returns a pooled connection to the sql database. The connection is
        /// returned to the pool when the pooled connection is dropped.
        ///
        /// **Panics** if a `DieselMiddleware` has not been registered with
        /// Iron, or if retrieving a connection to the database times out.
